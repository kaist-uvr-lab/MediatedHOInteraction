...
async def image_handler(websocket, path):
    ...
    for i, img in enumerate(img_set):
        img_name = "C:/Woojin/research/wiseui-ikbeom/Integration/WiseUIServer/records/save_4/" + str(i) + ".png"
...
def encode_hand_data(hand_result):
    ...
    for id in range(num_joints):
        joint = dict()
        joint['id'] = int(id)
        joint['u'] = round(float(hand_result[id, 0]), 3)
        joint['v'] = round(float(hand_result[id, 1]), 3)
        joint['d'] = round(float(hand_result[id, 2]), 3)
        joints.append(joint)
