...
namespace MRTK.Tutorials.GettingStarted
{
    public class PartAssemblyController : MonoBehaviour
    {
        ...
        private Transform partsParent = default;
        ...
        private void Awake()
        {
            GameObject roverAssembly = GameObject.FindGameObjectWithTag("RoverAssembly");
            if (roverAssembly != null)
            {
                PlacementHintsController placementHintsController = roverAssembly.GetComponent<PlacementHintsController>();
                if (placementHintsController != null)
                {
                    partsParent = placementHintsController.PartsParent;
                    GameObject[] placementHints = placementHintsController.PlacementHints;
                    foreach (GameObject hint in placementHints)
                    {
                        ParseAndCompareName(hint);
                    }
                }
            }
        ...
        private void ParseAndCompareName(GameObject hint)
        {
            string objectName = hint.name;
            int underscoreIndex = objectName.IndexOf('_');
            if (underscoreIndex >= 0)
            {
                string targetName = gameObject.name.Replace("(Clone)", "");
                string parsedName = objectName.Substring(0, underscoreIndex);

                if (parsedName == targetName)
                {
                    //Debug.Log("Parsed name matches target name for object: " + targetName + " vs " + parsedName);
                    locationToPlace = hint.transform;
                }
                else
                {
                    //Debug.Log("Parsed name does not match target name for object: " + targetName + " vs " + parsedName);
                }
            }
            else
            {
                //Debug.Log("No underscore found in object name: " + objectName);
            }
        }
