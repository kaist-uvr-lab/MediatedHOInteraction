...
from modules import HandTracker_our_v2, GestureClassfier, identify_interacting_finger
...
def main():
    ...
    try:
        while True:
            ...
            ###################### process hand ######################
            results = track_hand.run(np.copy(color))

            if not results:
                continue
            else:
                right, uvds, _, _ = results
                if not isinstance(uvds, np.ndarray):
                    continue
            ...
            indices = np.where(right == 1)[0]
            uvd_right = uvds[indices]
